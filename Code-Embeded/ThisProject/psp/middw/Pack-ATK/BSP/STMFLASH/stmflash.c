...
static __align(8)  uint64_t STMFLASH_BUF[STM_SECTOR_SIZE / 8]; //最多是2K字节
void STMFLASH_Write(uint32_t WriteAddr, uint8_t *pBuffer, uint32_t NumToWrite)
{
    ...
    while(1)
    {
        STMFLASH_Read(secpos * STM_SECTOR_SIZE + STM32_FLASH_BASE, (uint32_t*)STMFLASH_BUF, STM_SECTOR_SIZE / 4);	//读出该页全部数据
