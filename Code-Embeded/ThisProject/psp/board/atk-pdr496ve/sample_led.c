...
sttLedCtrl_t g_samp_leds[LedID_Max] = {
    ...
    [LedID_Red] = {
        .mID = LedID_Red,.mGpioBank_p =  GPIOE, .mPin = GPIO_PIN_7,
        .mSwhLvl_On = G_GPIO_PIN_RST,//低电平点亮
        .mSwhLvl_Off = G_GPIO_PIN_SET,//高电平熄灭
    },

    [LedID_Green] =  {
        .mID = LedID_Green,.mGpioBank_p =  GPIOE,  .mPin = GPIO_PIN_8,
        .mSwhLvl_On = G_GPIO_PIN_RST,//低电平点亮
        .mSwhLvl_Off = G_GPIO_PIN_SET,//高电平熄灭      
    },
#if   THIS_BOARD ==  G_ATK_PDR496VE
    [LedID_Blue] =  {
        .mID = LedID_Blue,.mGpioBank_p =  GPIOE,  .mPin = GPIO_PIN_9,
        .mSwhLvl_On = G_GPIO_PIN_RST,//低电平点亮
        .mSwhLvl_Off = G_GPIO_PIN_SET,//高电平熄灭      
    },
 ...
 *************
    for(emLedId_t  i = 0; i < LedID_Max; i++)
    {
        if(IS_VALID_ID_LED(i)){
			...
			API_PRP_GPIO_PIN_WT(pLedsCtrl[i].mGpioBank_p, pLedsCtrl[i].mPin, pLedsCtrl[i].mSwhLvl_Off);
        }
        else{
            return -2;
        }
...
char app_samp_leds_init(sttLedCtrl_t *pLedsCtrl)
{
...
#if THIS_BOARD  ==  G_ATK_PDR496VE
    GPIO_CLK_ENABLE_LED_RED();
		GPIO_CLK_ENABLE_LED_GREEN();
		GPIO_CLK_ENABLE_LED_BLUE();
#endif 
    ...
    for(emLedId_t  i = 0; i < LedID_Max; i++)
    {
        if(IS_VALID_ID_LED(i)){
          tGpioInit.Pin = pLedsCtrl[i].mPin;
					API_PRP_GPIO_INIT(GPIO_PORT_LED_RED, &tGpioInit);
		     	API_PRP_GPIO_PIN_WT(pLedsCtrl[i].mGpioBank_p, pLedsCtrl[i].mPin, pLedsCtrl[i].mSwhLvl_Off);
        }
        else{
            return -2;
