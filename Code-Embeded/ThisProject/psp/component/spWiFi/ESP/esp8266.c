 ...
 * @attention
       1  Add below int-defs to HAL_UART_MspInit() in usart.c
         ...
         3.1 app_esp_devgpios_init must add WIFI_MODE_WORK();  
             otherwise the espdev keeps in download mode
				 3.2 If the espdev has been restored to factory settings() using app_espdev_restore()
				     it is necessary to first set the device's cwmode using app_esp_set_cwmode()
				 ...
				 3.4 Link route AP
				    Successfully set the cwmode at first,and then use app_esp_link_ap()/app_esp_link_ap_v2()
         3.5 smartconfig
				 ...
				 4.2 Aliyun Living(飞燕平台-云智能app)
				   4.2.1 产品信息-> 产品类型选网关，联网方式选蜂窝，模组选其他
					...
					4.2.4 云智能app扫码配网二维码，只能连上面板界面，而不能连routeAP，因此如果连新的路由
					     仍需 smartconfig 联网 
       5 apis
			   5.1 Recommend using app_atkmodule_printf() instead of unusing app_esp_send_at_cmd()
       6			 
 ****************************************************************************************************
 * @note
 *    1 Default hard
...
//step1,necessary, reset the espdev by software-------------------------------  				
		ret = app_espdev_sftrst();          //软件复位 
		printf("Step%d,reset %s by codes...\r\n",step++,mqttdev->devname);
		if(ret != Err_WIFI_NONE){
			 printf("Err(%d) in app_espdev_sftrst()\r\n",ret);
		}	

//step2,necessary, set workmode-----------------------------------------------			
		ret = app_esp_set_cwmode(mqttdev->sta_cwm);   // Station模, TCP-client
		printf("Step%d,set %s to station...\r\n",step++,mqttdev->devname);
		if(ret != Err_WIFI_NONE){
			 printf("Err(%d) app_esp_set_cwmode()\r\n",ret);
		}		
...
//				app_esp_set_cwmode(EspCWM_Sta);
		  	app_esp_set_cwmode(EspCWM_StaAp);
				if(Err_WIFI_NONE == app_esp_smartconfig(mqttdev))
				{
					stkdly_ms(150);
...
//step4,necessary,check cipstate-------------------------------------------------	
    printf("Step%d stop smartconfig and check cipstatus in app_mqtt_init_v2\r\n",step++);
		app_esp_stop_smartconfig(mqttdev);		
    if(Err_WIFI_NONE == app_esp_cipstatus(mqttdev))
		{
  ...
  if(True == g_dbcom_rxtkn_ovr)
	{
		...
		{
			app_espdev_restore_v2(mqttdev);
		}
				
		if(NULL != strstr((sc8_t*)g_dbcom_rxbuff,"smartconfig"))
		{
			app_esp_set_cwmode(EspCWM_Sta);
			app_esp_smartconfig(mqttdev);
			app_atkmodule_uartrxmsg_poll();
		}
...
{
  __IO static emWIFI_DevState_t ret = WIFI_Disconnect ;
  ret	=  mqttdev->sta_dev;
	...
	if(True == g_atkmodule_rxtkn_ovr)
	{
		...
		{
			 ret = mqttdev->sta_dev = WIFI_Connect;	
			 app_atkmodule_uartrxmsg_poll();
		}	
 ...
 * @notice 
		 ...
		 -2-,After successfully running this func once, it cannot be run again. 
				If need to run it again,  must reset espdev at first ,then set up the cwmode
 ***************************************************************************/
uint8_t app_esp_smartconfig(sttMqtt_DevParas_t* mqttdev)
{
	uint8_t ret = Err_WIFI_ERROR;
...
emWIFI_DevState_t app_esp_cipstatus(sttMqtt_DevParas_t* mqttdev)
{
  ...
  {
    app_espdev_linksta_poll(mqttdev);
		ret = Err_WIFI_NONE;
	}			
...
{
  uint8_t tmp = 0;
	emWIFI_DevState_t ret = WIFI_Disconnect;
 ...
 *******************************************************************/
uint8_t app_esp_link_ap_v2(sttMqtt_DevParas_t* mqttdev,uint16_t ms)
{
	uint8_t ret;
	char cmd[56];
  if(ms < 4700)
...
{  
    ...
    WIFI_MODE_GPIO_CLK_ENABLE();    
    gpio_init_struct.Pin   = WIFI_MODE_GPIO_PIN;
    HAL_GPIO_Init(WIFI_MODE_GPIO_PORT, &gpio_init_struct);	
 ...
 * @retval      Err_WIFI_NONE  : ESPDev初始化成功，函数执行成功
 *              Err_WIFI_ERROR: ESPDev初始化失败，函数执行失败
 ***********************************************************************
 *   app_atkmodule_init(baudrate);  ESPDev UART初始化 
    if (app_esp_at_test() != Err_WIFI_NONE)  
    {
        return Err_WIFI_ERROR;
    }
...
{
    app_esp_devgpios_init();                  /* ESPDev硬件初始化 */
    app_espdev_hdrst();                         /* ESPDev硬件复位 */
	  app_atkmodule_uartrx_rst();
    return Err_WIFI_NONE;
}
 ...
 **************************************************************
  printf("v1Restore\r\n");
 ********************************************************************/
uint8_t app_espdev_restore(void)
{
	uint8_t ret = Err_WIFI_ERROR;
	uint8_t tmp = 0;
	...
	{
		app_atkmodule_printf("AT+RESTORE\r\n");
	  stkdly_ms(__DlyMs_Mqtt - __DlyMs_Esp);
 ...
 ************************************************************
  ret = app_esp_send_at_cmd(cmd,"OK",__DlyMs_Esp);
 ******************************************************************/
uint8_t app_esp_set_cwmode(emEsp8266CWMODE_t mode)
{
	uint8_t ret = 0;
  char cmd[16]= {0};
	...
	{
		printf("ErrPara(%d)_cwmode in app_esp_set_cwmode!!!!\r\n",mode);
...
printf("v1Reset\r\n");
 *********************************************************/
uint8_t app_espdev_sftrst(void)
{
	uint8_t ret = Err_WIFI_ERROR;
	uint8_t tmp = 0;
 ...
 *********************************************************
	ret = app_esp_send_at_cmd(cmd,"WIFI GOT IP",ms);
	if(ret != Err_WIFI_NONE){
		return Err_WIFI_ERROR;		
	}	else {
		app_atkmodule_uartrxmsg_poll();
 ...
 ********************************************************/
uint8_t app_esp_link_ap(char *ssid, char *pwd,uint16_t ms )
{
	uint8_t ret;
	char cmd[64];
	if(ms <= 4800)
...
//step2,necessary, set workmode-----------------------------------------------			
		ret = app_esp_set_cwmode(EspCWM_Sta);   // Station模, TCP-client
...
//step4,necessary, connect AP--------------------------------------------------  										
		...
		ret = app_esp_link_ap(username, password,0); // ("EDW201X","txhd349874");								
		if(ret == Err_WIFI_NONE){
		  	g_atkmoudle_esp_dev0.sta_dev = WIFI_Connect;
			  app_atkmodule_uartrxmsg_poll(); 				
		}else{
			 printf("Err(%d) app_esp_link_ap()\r\n",ret);
		}		
