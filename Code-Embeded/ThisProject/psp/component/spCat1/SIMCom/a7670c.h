...
#include SYS_HEAD
#include RES_CAT1_HEAD        //  "./component/spCat1/SIMCom/res_cat1.h"
#include RES_CAT1_UART_HEAD   // "./component/spCat1/SIMCom/resdrv_cat1_uart.h"
#include UTY_CAT1_HEAD       //"./component/spCat1/UTY/utyCat1.h"


void app_cat1_devgpios_init(void);

/* 操作函数 */
void app_cat1dev_hdrst(void);                                            /* ESPDev硬件复位 */
uint8_t app_cat1_send_at_cmd(char *cmd, char *ack, uint32_t timeout);    /* ESPDev发送AT指令 */
uint8_t app_atkmodule_esp_init(void);                                /* ESPDev初始化 */
uint8_t app_cat1dev_restore(void);                                          /* ESPDev恢复出厂设置 */
uint8_t app_cat1_at_test(void);                                          /* ESPDev AT指令测试 */
uint8_t app_cat1_set_cwmode(emEsp8266CWMODE_t mode);                                 /* 设置ESPDev工作模式 */
uint8_t app_cat1dev_sftrst(void);                                         /* ESPDev软件复位 */
uint8_t app_cat1_ate_config(uint8_t cfg);                                /* ESPDev设置回显模式 */
uint8_t app_cat1_link_ap(char *ssid, char *pwd, uint16_t);                         /* ESPDev连接WIFI */
uint8_t app_cat1_get_ip(char *buf);                                      /* ESPDev获取IP地址 */
uint8_t app_cat1_connect_tcp_server(char *server_ip, char *server_port); /* ESPDev连接TCP服务器 */
uint8_t app_cat1_enter_serialnet(void);                                /* ESPDev进入透传 */
void app_cat1_exit_serialnet(void);                                    /* ESPDev退出透传 */
uint8_t app_cat1_connect_atkcld(char *id, char *pwd);                    /* ESPDev连接原子云服务器 */
uint8_t app_cat1_disconnect_atkcld(void);                                /* ESPDev断开原子云服务器连接 */

/*********************************************************************************
 * @brief  (verified)Poll-proccess for UART/USART rx of LM`s bluetooth&ble dev 
...
///--drv-v2---------------------------------------------------------------------------------
/************************************************************************************
 * @brief (verified) pubulish Json-msg to hosturl ver2         
 * @param  mqttdev
 ...
 * @notice 
 *******************************************/
uint8_t app_cat1_smartconfig(sttMqtt_DevParas_t*);
...
#define app_cat1_autoconn_enable()   app_cat1_autoconn_swh(true)
#define app_cat1_autoconn_disable()  app_cat1_autoconn_swh(false)	
...
/********************************************************************
 * @brief  (verified)ESPDev links RouteAP
           [+] smartconfig
 * @param  mqttdev
 ...
 *******************************************************************/
uint8_t app_cat1_link_ap_v2(sttMqtt_DevParas_t* mqttdev,uint16_t);
...
/************************************************************************************
 * @brief (verified) pubulish Json-msg of tpp(thing property post) to hosturl 
...
/**********************************************************************
 * @brief (verified) Analyze message from Aliyun Ling App(云智能)
