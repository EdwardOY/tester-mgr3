...
#include RES_CAT1_UART_HEAD
#include UTY_CAT1_HEAD      // "./component/spCat1/UTY/utyCat1.h"

///==ESP->paras==============================================================================================
#define __DlyMs_Cat1    (uint8_t)70

//---WiFidev_RstGpio------------------------------------------------------------------------------
#define AKM_RST_GPIO_PORT           GPIOA
#define AKM_RST_GPIO_PIN            GPIO_PIN_4
#define AKM_RST_GPIO_CLK_ENABLE()   __HAL_RCC_GPIOA_CLK_ENABLE()
#define AKM_RST_GPIO(x)    do{ x ?                                                                      \
																HAL_GPIO_WritePin(AKM_RST_GPIO_PORT, AKM_RST_GPIO_PIN, GPIO_PIN_SET) :  \
																HAL_GPIO_WritePin(AKM_RST_GPIO_PORT, AKM_RST_GPIO_PIN, GPIO_PIN_RESET); \
                            }while(0)

#define AKM_RST_EN()       AKM_RST_GPIO(0)  
#define AKM_RST_DIS()      AKM_RST_GPIO(1)  
//--End WiFidev_RstGpio-------------------------------------------------------------------End-----	
														
//--DownloadGpio----------------------------------------------------------------------------------										
#define AKM_PWR_GPIO_PORT           GPIOI
#define AKM_PWR_GPIO_PIN            GPIO_PIN_11
#define AKM_PWR_GPIO_CLK_ENABLE()   __HAL_RCC_GPIOI_CLK_ENABLE()
#define AKM_PWR_GPIO(x)    do{ x ?                                                                      \
																HAL_GPIO_WritePin(AKM_PWR_GPIO_PORT, AKM_PWR_GPIO_PIN, GPIO_PIN_SET) :  \
																HAL_GPIO_WritePin(AKM_PWR_GPIO_PORT, AKM_PWR_GPIO_PIN, GPIO_PIN_RESET); \
                            }while(0)

#define AKM_PWR_ON()      AKM_PWR_GPIO(0) //when download espdev firmware
#define AKM_PWR_OFF()     AKM_PWR_GPIO(1) // when espdev runs(serialnet or mqtt ...)		
//--End DownloadGpio-----------------------------------------------------------------------End-----													
///---End ESP-dev-gpio--------------------------------------------------------------------------------End---

//---ESP-dev-paras---------------------------------------------------------------------------------
//CWMODE para,called by app_cat1_set_cwmode in esp8266.c
typedef enum{
...
/*******************************************
  ...
  * thg == thing
  ******************************************/
//verified Aliyun Living IoT thing->property
//true-->open; false-->close
